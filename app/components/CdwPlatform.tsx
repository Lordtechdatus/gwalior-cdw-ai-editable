...
type Screen = "landing" | "roles" | "workspace";
type ReportStatus =
  | "Submitted"
  | "Processing"
  | "Scheduled"
...
export default function CdwPlatform({ viewer }: { viewer: Viewer }) {
  ...
  function enterRole(selectedRole: Role) {
  ...
  return (
    <div className="min-h-screen bg-[#07100f] text-[#f3fbf8]">
      {screen === "landing" && (
        <Landing onStart={() => setScreen("roles")} viewer={viewer} />
      )}
    ...
    <main className="grid-surface min-h-screen px-5 py-7 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="flex items-center justify-between">
          <button
            onClick={onBack}
            className="inline-flex items-center gap-2 text-sm font-medium text-[#8aa49d] transition hover:text-white"
          >
        ...
        <div className="mx-auto mt-16 max-w-3xl text-center">
          ...
          <h1 className="mt-4 text-4xl font-semibold tracking-tight text-white sm:text-5xl">
            Choose an operational workspace
          </h1>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[#89a099] sm:text-base">
            This prototype lets you inspect each role. In production, the role is assigned by an
            administrator and enforced on the server.
          </p>
        </div>
        ...
        <div className="mx-auto mt-8 flex max-w-xl items-center justify-center gap-3 rounded-2xl border border-white/8 bg-white/[0.025] px-4 py-3 text-xs text-[#78958e]">
          ...
          Signed in as <span className="font-semibold text-[#c7d9d4]">{viewer.email}</span>
        </div>
      </div>
    </main>
  );
    ...
    <div className="min-h-screen bg-[#07100f] lg:grid lg:grid-cols-[248px_1fr]">
      ...
      >
        ...
        <div className="border-t border-white/8 pt-4">
          ...
          >
            <LogOut className="h-[18px] w-[18px]" /> Switch workspace
          </button>
        </div>
      </aside>
